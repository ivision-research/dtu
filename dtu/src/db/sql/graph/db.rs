...
use crate::db::graph::{ClassSpec, Error as GraphError, GraphDatabase, Result as GraphResult};
use crate::db::sql::common::DBThread;
use crate::db::sql::graph::models::Source;
...
impl From<diesel::result::Error> for GraphError {
...
impl GraphSqliteDatabase {
    ...
    fn get_calls(
        ...
        depth: usize,
    ) -> GraphResult<Vec<MethodCallPath>> {
    ...
    method_calls(source, class, name, args, ret, idx) AS (
        SELECT s.name, c.name, m.name, m.args, m.ret, CAST(p.key AS INTEGER)
        FROM calls_to AS ct
...
impl GraphDatabase for GraphSqliteDatabase {
    ...
    fn wipe(&self, ctx: &dyn Context) -> GraphResult<()> {
        let path = ctx.get_sqlite_dir()?.join(GRAPH_DATABASE_FILE_NAME);
        std::fs::remove_file(&path)?;
        Ok(())
    }
    fn remove_source(&self, source: &str) -> GraphResult<()> {
        Ok(self.delete_source_by_name(source)?)
    }

    fn get_all_sources(&self) -> GraphResult<HashSet<String>> {
    ...
    fn get_classes_for(&self, source: &str) -> GraphResult<Vec<ClassName>> {
        self.with_connection(|c| -> GraphResult<Vec<ClassName>> {
    ...
    fn get_methods_for(&self, source: &str) -> GraphResult<Vec<MethodSpec>> {
        self.with_connection(|c| -> GraphResult<Vec<MethodSpec>> {
            ...
            .load::<(String, String, String, String, String)>(c)?;
            Ok(rows
                .into_iter()
                .map(|(class, name, signature, ret, source)| MethodSpec {
                    class: class.into(),
                    name,
                    signature,
                    ret,
    ...
    fn find_child_classes_of(
        ...
        parent: &ClassSearch,
        source: Option<&str>,
    ) -> GraphResult<Vec<ClassSpec>> {
        self.with_connection(|c| -> GraphResult<Vec<ClassSpec>> {
