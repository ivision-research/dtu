...
use crate::db::graph::{AddDirTask, InitialImportOptions};
use crate::db::sql;
use crate::db::sql::graph::models::{InsertClass, InsertLoadStatus, InsertSource};
use crate::db::sql::graph::schema::{_load_status, classes, sources};
use crate::utils::DevicePath;
use crate::{
    db::graph::{
        setup::SetupResult, AddDirectoryOptions, Error as GraphError, GraphDatabaseSetup,
        LoadCSVKind, Result as GraphResult, SetupEvent,
    },
...
impl<'a> SetupContext<'a> {
    ...
    fn stage_methods(self) -> GraphResult<()> {
        self.do_load(|c, record| -> sql::Result<()> {
            ...
            let sql = "INSERT INTO named_methods(class, name, args, ret) VALUES(?, ?, ?, ?)";
            query!(sql_query(sql)
                .bind::<Text, _>(class)
                .bind::<Text, _>(name)
                .bind::<Text, _>(args)
    ...
    fn load_classes(self) -> GraphResult<()> {
        let src = self.source;
        self.do_load(|c, record| -> sql::Result<()> {
    ...
    fn stage_calls(self) -> GraphResult<()> {
        self.do_load(|c, record| -> sql::Result<()> {
    ...
    fn stage_supers(self) -> GraphResult<()> {
        self.do_load(|c, record| -> sql::Result<()> {
    ...
    fn stage_impls(self) -> GraphResult<()> {
        self.do_load(|c, record| -> sql::Result<()> {
    ...
    fn do_load<F>(self, f: F) -> GraphResult<()>
    where
        F: Fn(&mut SqliteConnection, &StringRecord) -> sql::Result<()> + Send,
    {
        Ok(self.db.transaction(move |c| -> sql::Result<()> {
            let mut record = StringRecord::new();

            loop {
                match self.data.read_record(&mut record) {
                    Err(_) if self.data.is_done() => break,
                    Err(e) => return Err(sql::Error::Generic(e.to_string())),
                    _ => {}
                }
...
JOIN classes AS interface
    ...
    SELECT DISTINCT c.id, nm.name, nm.args, nm.ret, ?1
...
WHERE dst.id != src.id"#
    ...
    fn load_staged_impls(&self, src: i32) -> GraphResult<()> {
        Ok(self.transaction(|c| self.load_staged_impls_with_conn(c, src))?)
    }

    fn load_staged_supers(&self, src: i32) -> GraphResult<()> {
        Ok(self.transaction(|c| self.load_staged_supers_with_conn(c, src))?)
    }

    fn load_staged_methods(&self, src: i32) -> GraphResult<()> {
        Ok(self.transaction(|c| self.load_staged_methods_with_conn(c, src))?)
    }

    fn load_staged_calls(&self, src: i32) -> GraphResult<()> {
...
impl GraphDatabaseSetup for GraphSqliteDatabase {
    ...
    fn load_begin(&self, _ctx: &dyn Context) -> GraphResult<()> {
...
"#,
    ...
    fn load_complete(&self, _ctx: &dyn Context, _success: bool) -> GraphResult<()> {
        self.with_connection(|c| {
            c.batch_execute(
                r#"
...
impl<'a> RecordParser<'a> {
    ...
    fn get(&self, idx: usize) -> sql::Result<&'a str> {
        self.record.get(idx).ok_or_else(|| {
            sql::Error::Generic(format!("invalid {}, missing string at {}", self.name, idx))
        })
    }
    fn get_parsable<T>(&self, idx: usize) -> sql::Result<T>
    where
        T: FromStr,
    {
        let val = self.get(idx)?;
        str::parse::<T>(val).map_err(|_| {
            sql::Error::Generic(format!(
...
impl<'a> InsertClass<'a> {
    fn from_record(record: &'a StringRecord, src: i32) -> sql::Result<Self> {
