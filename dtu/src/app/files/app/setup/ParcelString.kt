...
private fun maybeCall(intent: Intent, name: String, vararg args: Any): Boolean {
    if (!name.startsWith("#m:")) {
        return false;
    }
    val methodName = name.substring(3)
    val clazz = intent::class
    val method = clazz.members.find {
        it.name == methodName && it.parameters.size == args.size + 1
    } ?: return false;
    alogv("Invoking Intent.${methodName}")
    method.call(intent, *args)
    return true
}
...
sealed class Value() {
    ...
    class VMessage(val value: Message) : Value() {
        companion object {
            @JvmStatic
            fun fromString(s: String): VMessage {
                val split = SimpleEscapeSplitter('%').split(s)
                val iter = split.iterator()
                val what = Integer.parseInt(iter.next())
                val arg1 = Integer.parseInt(iter.next())
                val arg2 = Integer.parseInt(iter.next())
                val msg = Message.obtain()
                msg.what = what
                msg.arg1 = arg1
                msg.arg2 = arg2
                if (iter.hasNext()) {
                    val bund = VBundle.fromString(iter.next())
                    msg.data = bund.value
                }
                return VMessage(msg)
            }
        ...
        override fun addToContentValues(key: String, into: ContentValues) {
            throw UnsupportedContentValuesUsage("Message")
        }
        override fun addToIntent(key: String, intent: Intent) {
            throw UnsupportedIntentUsage("Message")
        }
        override fun writeToParcel(dest: Parcel, flags: Int) {
            value.writeToParcel(dest, flags)
        }
...
enum class ParcelStringPrefix(val prefix: String, val clazz: KClass<out Value>) {
    ...
    VMessage("_MSG", Value.VMessage::class),
